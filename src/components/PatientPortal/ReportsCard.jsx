...
const ReportsCard = () => {
  ...
  return (
    <div className="bg-white rounded-2xl p-4 sm:p-6 shadow-lg w-full">
      ...
      <button className="w-full bg-gradient-to-r from-blue-500 to-cyan-500 text-white rounded-xl py-2 sm:py-3 px-3 sm:px-4 font-semibold flex items-center justify-center gap-1 sm:gap-2 hover:from-blue-600 hover:to-cyan-600 transition-all mb-4 sm:mb-6">
        Upload New Report
      </button>
      ...
      <div className="space-y-2 sm:space-y-3">
        {reports.map(report => (
          ...
          >
            <span className="text-gray-700 text-sm sm:text-base">{report.name}</span>
            <button className="mt-1 sm:mt-0 text-blue-500 hover:text-blue-600 text-sm sm:text-base">
              Download
            </button>
          </div>
        ))}
