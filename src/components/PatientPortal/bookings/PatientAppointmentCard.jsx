...
import RescheduleCart from "./RescheduleCart";
import { rescheduleAppointment } from "../../../api/api";
...
export default function AppointmentCard({
  ...
  const confirmCancel = () => {
    if (onRemove) onRemove(appointment.appointmentId);
    ...
    <div className="border rounded p-4 shadow-md bg-white flex flex-col gap-2">
      ...
      {rescheduleMode && (
        <RescheduleCart
          ...
          onSubmit={async (appointmentId, newSlotId) => {
            try {
              await rescheduleAppointment(appointmentId, newSlotId);
              alert("Appointment rescheduled successfully!");
              onReschedule && onReschedule(appointmentId, newSlotId);
